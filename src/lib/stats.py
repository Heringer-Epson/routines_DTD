...
def binned_DTD_rate(s1, s2, t0, tc):
    """Computes the average SN rate in each time bin."""
    #Assumes that t_cutoff happens in this bin. True for most sensible cases.
    #t_cutoff usually tested in the range 0.5--2 Gyr.
    if (abs(s1 + 1.) > 1.e-6) and (abs(s2 + 1.) > 1.e-6):
        psi1 = 1. / (s1 + 1.) * (0.42**(s1 + 1.) - t0**(s1 + 1.)) / (0.42 - t0)
        psi2 = (1. / (2.4 - 0.42)
          * ((tc**(s1 + 1.) - 0.42**(s1 + 1.)) / (s1 + 1.)
          + (tc**(s1 - s2) / (s2 + 1.) * (2.4**(s2 + 1.) - tc**(s2 + 1.)))))
        psi3 = 1. / (s2 + 1.) * (14.**(s2 + 1.) - 2.4**(s2 + 1.)) / (14. - 2.4)    
    elif (abs(s1 + 1.) < 1.e-6) and (abs(s2 + 1.) > 1.e-6):
        psi1 = np.log(0.42 / t0) / (0.42 - t0)
        psi2 = (1. / (2.4 - 0.42)
          * (np.log(2.4 / 0.42) 
          + (tc**(s1 - s2) / (s2 + 1.) * (2.4**(s2 + 1.) - tc**(s2 + 1.)))))
        psi3 = 1. / (s2 + 1.) * (14.**(s2 + 1.) - 2.4**(s2 + 1.)) / (14. - 2.4)       
        ...
        psi2 = (1. / (2.4 - 0.42)
          * ((tc**(s1 + 1.) - 0.42**(s1 + 1.)) / (s1 + 1.)
          + (tc**(s1 - s2) * np.log(2.4 / 0.42))))
        psi3 = np.log(4. / 2.4) / (14. - 2.4)  
    elif (abs(s1 + 1.) < 1.e-6) and (abs(s2 + 1.) < 1.e-6): #i.e. s1=s2=-1
        psi1 = np.log(0.42 / t0) / (0.42 - t0)
        psi2 = np.log(2.4 / 0.42) / (2.4 - 0.42)
        psi3 = np.log(14. / 2.4) / (14. - 2.4)   
    return psi1, psi2, psi3

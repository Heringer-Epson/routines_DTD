...
class DTD_Approximations(object):
    ...
    def loop_t_onset(self):
        ...
        for i, t_ons in enumerate(t_onset):            
            ...
            #Fit the derived values in linear scale.
            popt, pcov = curve_fit(power_law, x, psi_values)
            y_fit = power_law(x_fit, popt[0], popt[1])
            ...
            self.ax2.errorbar(
              np.log10(popt[0]), popt[1], xerr=np.sqrt(np.diag(pcov))[0] / popt[0],
    ...
    def manage_output(self):
        plt.tight_layout()
        if self.save_fig:
            fpath = './../OUTPUT_FILES/ANALYSES_FIGURES/Fig_M12-method_test2.pdf'
